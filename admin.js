...

{
  ...
  document.getElementById('adminName').textContent = nombreCapitalizado || 'Administrador';
  ...
  async function cargarDashboard() {
    try {
      ...
      const recentUsersHtml = usuarios.slice(0, 5).map(u => {
        const nombreCap = capitalizarNombre(u.nombre);
        return `
          <div class="recent-item">
            <div class="recent-avatar">${nombreCap.charAt(0).toUpperCase()}</div>
      ...
      const actividadHtml = `
        <div class="activity-item">
          <strong>${nombreCapitalizado}</strong> inició sesión como administrador
